...
class Intranet_model extends CI_Model
{
  ...
  public function check_pass_admin($pass, $iduser)
  {
    $q = $this
      ->db
      ->select('*')
  ...
  public function check_pass_intranet($pass, $iduser)
  {
    $q = $this
      ->db
      ->select('*')
  ...
  public function get_intranet_usuarios()
  {
    ...
    $q= $this
        ...
        ->where('intranet_usuarios.status', 1)
        ->get();
    if($q->num_rows() > 0) {
      return $q->result_array();
    }
  ...
  {
    $str = 'elevadores.id, elevadores.form, elevadores.empresaid, elevadores.horario, ';
    $str .= 'torres.id_torres, torres.torre, ';
    $str .= 'oficinas.id_oficina, oficinas.nombre_oficina, oficinas.imagen_oficina, oficinas.email1';
	...
	public function get_solicitudes_elevadores_all_status($status)
  {
    $str = 'elevadores.id, elevadores.form, elevadores.empresaid, elevadores.horario, elevadores.status_elevador, ';
    $str .= 'torres.id_torres, torres.torre, ';
    $str .= 'oficinas.id_oficina, oficinas.nombre_oficina, oficinas.imagen_oficina, oficinas.email1';
    ...
    $q= $this
        ...
        ->from('elevadores')
        // ->join('torres', 'torres.id_torres = elevadores.torre_id')
				->where('elevadores.status_elevador', $status)
        ->join('oficinas', 'oficinas.id_oficina = elevadores.empresaid')
				->order_by('id', 'desc')
				//->limit(300) //TEMP
        ->get();
  ...
  public function get_solicitud_elevador($id)
  {
	...
	public function get_solicitud_elevador_status($id, $status)
  {
    $str = 'elevadores.id, elevadores.titulo, elevadores.empresaid, elevadores.torre_id, elevadores.oficina, elevadores.fechahora, elevadores.descripcion, elevadores.status_elevador, ';
    $str .= 'torres.id_torres, torres.torre, ';
    $str .= 'oficinas.id_oficina, oficinas.nombre_oficina, oficinas.imagen_oficina, oficinas.email1';
  ...
  public function get_solicitudes_mudanzas_files($idmudanza)
  {
    $q= $this
        ->db
        ->select('*')
        ->from('files_mudanza')
        ->where('idmudanza', $idmudanza)
        ->get();
    if($q->num_rows() > 0) {
      return $q->result_array();
    }
  ...
  public function get_archivo_pdf($id)
  {
    $q = $this
      ->db
      ->select('*')
