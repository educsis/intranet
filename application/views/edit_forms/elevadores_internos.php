<form class="internosForm formE" method="POST" action="<?= base_url('elevadores/editar_elevador/' . $idsol . '/' . $tiposol) ?>">
	<div class="form">
		<input type="hidden" name="solId" value="<?= $elevador_info[0]['id'] ?>">
		<?php
		$oficina = $this->intranet_model->get_oficina_single($_SESSION['idoficina']);
		?>
		<h2 class="text-success">Editar Trabajos internos</h2>
		...
		<div class="col-md-3 form-group">
			...
			<input type="date" name="fecha" value="<?= $elevador_info[0]['fecha'] ?>" class="form-control" required="true" autocomplete="off">
			</div>
		</div>
		<div class="col-md-3 form-group">
			...
			<option value="09:00 a 12:00 horas" <?= ($elevador_info[0]['horario'] == '09:00 a 12:00 horas')?'selected = "selected"':'' ?>>09:00 a 12:00 horas</option>
			<option value="14:00 a 17:00 horas" <?= ($elevador_info[0]['horario'] == '14:00 a 17:00 horas')?'selected = "selected"':'' ?>>14:00 a 17:00 horas</option>
			<option value="19:00 a 06:30 horas" <?= ($elevador_info[0]['horario'] == '19:00 a 06:30 horas')?'selected = "selected"':'' ?>>19:00 a 06:30 horas</option>
			</select>
		</div>
		<div class="col-md-3 form-group">
			...
			<option value="0">Selecciona una opción</option>
			<option value="Reparaciones" <?= ($elevador_info[0]['requerimiento'] == 'Reparaciones')?'selected = "selected"':'' ?>>Reparaciones</option>
			<option value="Mantenimiento" <?= ($elevador_info[0]['requerimiento'] == 'Mantenimiento')?'selected = "selected"':'' ?>>Mantenimiento</option>
			<option value="Remodelación" <?= ($elevador_info[0]['requerimiento'] == 'Remodelación')?'selected = "selected"':'' ?>>Remodelación</option>
			</select>
		</div>
		</div>
		<div class="row">
		...
		<div class="col-md-3 form-group">
			<label>* Email</label>
			<input type="email" name="autorizaemail" class="form-control" required="true" autocomplete="off" value="<?= $elevador_info[0]['autorizaemail'] ?>">
		</div>
		</div>
		<div class="form-layout-footer">
		<button type="submit" style="cursor: pointer;" name="editarElevadoresInternos" class="btn btn-info mg-r-5">Editar solicitud</button>
